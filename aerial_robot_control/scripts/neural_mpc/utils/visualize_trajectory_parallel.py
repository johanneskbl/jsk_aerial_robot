...
def plot_trajectory_parallel(
    model_options, rec_dict, nominal_mpc, neural_mpc, dist_dict=None, save=False
):
    figures = []
    ...
    state_prop_neural = rec_dict["state_prop_neural"]
    ...
    for dim in range(state_in_nominal.shape[1] - 1, -1, -1):
        ...
        plt.plot(timestamp, state_in_neural[:, dim], label="state_in_neural", alpha=0.5)
        plt.plot(timestamp, state_out_neural[:, dim], label="state_out_neural")
        plt.plot(timestamp, state_prop_neural[:, dim], label="state_prop_neural", alpha=0.5)
        plt.ylabel(f"D{dim}")
        if dim == 0:
    ...
    for dim in range(control_nominal.shape[1] - 1, -1, -1):
        ...
        plt.plot(timestamp, control_neural[:, dim], label="control_neural")
        if dim == 0:
            plt.legend()
        plt.grid("on")
        plt.xlim(timestamp[0], timestamp[-1])
    ...
    plt.plot(timestamp, state_in_neural[:, 5], label="state_in_neural")
    plt.plot(timestamp, state_out_neural[:, 5], label="state_out_neural")
    plt.plot(timestamp, state_prop_neural[:, 5], label="state_prop_neural")
    plt.grid("on")
    ...
    y_true_neural = (state_out_neural - state_prop_neural) / dt
    ...
    for dim in range(state_in_nominal.shape[1] - 1, -1, -1):
        ...
        plt.plot(timestamp, y_true_neural[:, dim], color="tab:green", label="neural")
        if dim == 0:
            plt.legend()
        plt.grid("on")
        plt.xlim(timestamp[0], timestamp[-1])
    ...
    dynamics = init_forward_prop(nominal_mpc, return_continuous=True)
    ...
    if nominal_mpc.include_cog_dist_parameter:
        lin_acc_dist_nominal = lin_acc_nominal + dist_dict["cog_dist_nominal"][1::2, :3] / nominal_mpc.phys.mass

    #######################################################################
    # Plot regression of neural network
    if neural_mpc.use_mlp:
        state_in_neural = state_in_nominal.copy()
        ...
        state_b_torch_nominal = (
            torch.from_numpy(state_b_nominal[:, neural_mpc.state_feats])
            .type(torch.float32)
            .to(neural_mpc.device)
        )
        control_torch_nominal = (
            torch.from_numpy(control_nominal[:, neural_mpc.u_feats])
            .type(torch.float32)
            .to(neural_mpc.device)
        )
        ...
        if neural_mpc.mlp_metadata["ModelFitConfig"]["label_transform"]:
            for t in range(state_in_neural.shape[0]):
                if set([3, 4, 5]).issubset(set(neural_mpc.y_reg_dims)):
                    v_idx = np.where(neural_mpc.y_reg_dims == 3)[0][
                        0
        ...
        loss = np.square(y_true_nominal[:, neural_mpc.y_reg_dims] - y)
        fig, _ = plt.subplots(figsize=(10, 5))
        ...
        # rmse = np.sqrt(total_loss / y_neural.shape[1])
        ...
        # x_dot = np.empty(state_in_neural.shape)
        # for t in range(state_in_neural.shape[0]):
        #     x_dot[t, :] = np.array(
        #         dynamics(x=state_in_neural[t, :], u=control_neural[t, :])["x_dot"]
        #     ).squeeze()
        # lin_acc_neural = x_dot[:, 3:6]
        # lin_acc_dist_neural = (
        #     lin_acc_neural + dist_dict["cog_dist_neural"][1::2, :3] / neural_mpc.phys.mass
        # )
        ...
        for i, dim in enumerate(neural_mpc.y_reg_dims):
            plt.subplot(len(neural_mpc.y_reg_dims), 1, i + 1)
            plt.plot(lin_acc_neural[:, i], label="Acceleration by undisturbed model", color="tab:blue")
            if nominal_mpc.include_cog_dist_parameter:
                plt.plot(
                    lin_acc_dist_neural[:, i], label="Acceleration by disturbed model", color="tab:olive"
                )
            # plt.plot(lin_acc_dist_neural[:, i] + y[:, i], label="Neural Compensation (+)", color="tab:brown")
            plt.ylabel(f"D{dim}")
            if i == 0:
                plt.legend()
            plt.grid("on")
    ...
    for i, dim in enumerate(neural_mpc.y_reg_dims):
        ...
        plt.plot(lin_acc_neural[:, i], label="Acceleration by undisturbed model", color="tab:red")
        plt.ylabel(f"D{dim}")
        if i == 0:
            plt.legend()
        plt.grid("on")
    ...
    for i, dim in enumerate(neural_mpc.y_reg_dims):
        plt.subplot(len(neural_mpc.y_reg_dims), 1, i + 1)
        if nominal_mpc.include_cog_dist_parameter:
            plt.plot(lin_acc_dist_neural[:, i], label="Acceleration by disturbed model", color="tab:olive")
        plt.plot(lin_acc_neural[:, i], label="Acceleration by undisturbed model", color="tab:red")
        plt.ylabel(f"D{dim}")
        if i == 0:
            plt.legend()

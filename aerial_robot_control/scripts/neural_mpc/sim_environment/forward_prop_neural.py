...
def init_forward_prop_neural(mpc: NeuralMPC, T_prop_step: float = 0.01, num_stages: int = 4, return_continuous: bool = False):
    """
    ...
    # Residual acceleration predicted by neural network
    a_res = ca.MX.sym("a_res", 3)
    assert a_res.shape[0] == 3, "Currently, only support for predicting residual linear acceleration in World frame (i.e., y_reg_dims = [3, 4, 5])"

    # Generate symbolic continuous CasADi function
    dynamics = full_dynamics_neural(mpc, state, control, a_res)
    if return_continuous:
        return dynamics
    
    # Discretize dynamics
    discretized_dynamics = discretize_dynamics_neural(dynamics, state, control, a_res, T_prop_step, num_stages)
    return discretized_dynamics

def forward_prop_neural(discretized_dynamics, state_0, u_cmd, a_res, T_prop_horizon, T_prop_step, const_input=True):
    """
    ...
    for k in range(N):
        u_k = u_cmd[k, :]
        state_k = np.array(discretized_dynamics(state_k, u_k, a_res))  # state_k+1
        state_prop = np.append(state_prop, state_k.T, axis=0)

    return state_prop

def discretize_dynamics_neural(dynamics, x, u, a_res, T_prop_step, num_stages):
    """
    ...
    for j in range(num_stages):
        m1 = dynamics(x=x, u=u, a_res=a_res)["x_dot"]
        m2 = dynamics(x=x + dt / 2 * m1, u=u, a_res=a_res)["x_dot"]
        m3 = dynamics(x=x + dt / 2 * m2, u=u, a_res=a_res)["x_dot"]
        m4 = dynamics(x=x + dt * m3, u=u, a_res=a_res)["x_dot"]
        x += dt / 6 * (m1 + 2 * m2 + 2 * m3 + m4)

    return ca.Function("F", [x0, u, a_res], [x], ["x0", "u", "a_res"], ["xf"])

def full_dynamics_neural(mpc: NeuralMPC, state, u, a_res):
    """
    ...
    return ca.Function("x_dot", [state, u, a_res], [x_dot], ["x", "u", "a_res"], ["x_dot"])
